...
def test_no_round_trip_on_linear_trail() -> None:
    ...
    trail = _linear_trail()
    stations = [
        (_make_station("A", 48.00, 2.00), 0.0),
        (_make_station("B", 48.00, 2.05), 0.5),
        (_make_station("C", 48.00, 2.10), 1.0),
    ]
    result = find_round_trip_hikes(trail, stations, min_step_distance_km=4.0, max_step_distance_km=20.0)
    assert result == []
...
def test_round_trip_returns_loop_on_circular_trail() -> None:
    ...
    stations = [
        (_make_station("A", 48.00, 2.00), 0.00),
        (_make_station("B", 48.00, 2.04), 0.25),
        (_make_station("C", 48.04, 2.04), 0.50),
        (_make_station("D", 48.04, 2.00), 0.75),
    ]
    result = find_round_trip_hikes(trail, stations, min_step_distance_km=2.0, max_step_distance_km=8.0)
